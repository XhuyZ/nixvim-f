...
}:
{
  plugins = {
    snacks = {
      settings = {
        gitbrowse.enabled = lib.elem "snacks-gitbrowse" config.xhuyzvim.git.integrations;
      };
    };
  };

  keymaps = lib.mkIf (lib.elem "snacks-gitbrowse" config.xhuyzvim.git.integrations) [
    {
      mode = "n";
      key = "<leader>go";
